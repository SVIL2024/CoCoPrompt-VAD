...
class AnomalyCLIPModule(LightningModule):
    ...
    def on_train_start(self):
        ...
        if ncentroid_file.is_file():
            # file exists, load
            self.ncentroid = torch.load(ncentroid_file, weights_only=False)
        else:
            with torch.no_grad():
    ...
    def on_validation_epoch_end(self):
        ...
        dataset_name = type(self.trainer.datamodule).__name__.lower()
        labels_file = str(self.trainer.datamodule.hparams.labels_file).lower()
        if "xd" in labels_file or "xd" in dataset_name:
            selection_score = auc_pr + mean_mc_aupr
        else:
            selection_score = auc_roc + mean_mc_auroc
        ...
        self.log("val/selection_score", selection_score, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        self.class_probs.extend(class_probs)
        self.abnormal_scores.extend(abnormal_scores)
        self.softmax_similarity.extend(softmax_similarity)
    ...
    def on_test_epoch_end(self):
        ...
        labels = torch.stack(self.labels)
        class_probs = torch.stack(self.class_probs)
        softmax_similarity = torch.stack(self.softmax_similarity)
        ...
        if self.trainer.ckpt_path is not None:
            ...
            save_dir = "default"
        save_dir = Path(self.trainer.default_root_dir) / "eval_results" / str(save_dir)
        if not save_dir.is_dir():
            save_dir.mkdir(parents=True, exist_ok=True)
        ...
        self.log("test/AUC", auc_roc, on_step=False, on_epoch=True, prog_bar=True, rank_zero_only=True)
        self.log("test/AP", auc_pr, on_step=False, on_epoch=True, prog_bar=True, rank_zero_only=True)
        self.log("test/mAUC", mean_mc_auroc, on_step=False, on_epoch=True, prog_bar=True, rank_zero_only=True)
        self.log("test/mAP", mean_mc_aupr, on_step=False, on_epoch=True, prog_bar=True, rank_zero_only=True)

        with open(save_dir / "metrics.json", "w") as fp:
            json.dump(metrics, fp, indent=4, sort_keys=True)

        np.savez_compressed(
            save_dir / "predictions_raw.npz",
            labels=labels.detach().cpu().numpy(),
            abnormal_scores=abnormal_scores.detach().cpu().numpy(),
            softmax_similarity=softmax_similarity.detach().cpu().numpy(),
            class_probs=class_probs.detach().cpu().numpy(),
            normal_id=np.array([normal_idx], dtype=np.int64),
        )
        ...
        macro_recall = float(torch.diag(confmat).mean().cpu().data.numpy())
        fig = plt.figure(figsize=(28, 24))
        ax = plt.subplot()  # /np.sum(cm)
        f = sns.heatmap(
            confmat.cpu().data.numpy(),
            annot=True,
            ax=ax,
            fmt=".2%",
            cmap="Blues",
        ...
        ax.set_xlabel("Predicted", fontsize=52)
        ax.xaxis.set_label_position("bottom")
        plt.xticks(rotation=90)
        ax.xaxis.set_ticklabels(class_names, fontsize=32)
        ax.xaxis.tick_bottom()
        #
        ax.set_ylabel("True", fontsize=60)
        ax.yaxis.set_ticklabels(class_names, fontsize=32)
        plt.yticks(rotation=0)

        summary = f"AP↑={auc_pr:.4f}  |  MacroRecall↑={macro_recall:.4f}"
        fig.text(0.5, 0.02, summary, ha="center", va="center", fontsize=40, fontweight="bold")
